...
def segment(img):
    ...
    for i in range(detection_count):
        ...
        y2 = int(box[6] * h)
        roi = img[y1: y2, x1: x2]
        img, landm = detector.findPose(img, roi, x1, y1)
        if (landm):
            text = pose_est(landm)
            cv2.putText(img, text, (int((x1+x2)/2), y1+10), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
    return img


class poseDetector():
    def __init__(self, mode=False, modelComplex=1, smooth_lm=True,
                 en_seg=False, smooth_seg=True, detection_con=0.8, track_con=0.5):
        self.mode = mode
